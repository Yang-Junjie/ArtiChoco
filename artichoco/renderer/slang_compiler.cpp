...
ShaderReflection reflectProgram(slang::ProgramLayout& layout, ShaderStageMask stages)
{
    ...
    for (uint32_t parameter_index = 0; parameter_index < layout.getParameterCount(); ++parameter_index) {
        ...
        for (SlangInt range_index = 0; range_index < range_count; ++range_index) {
            ...
            if (binding_type == slang::BindingType::PushConstant) {
                ...
                const size_t size = value_layout == nullptr ? type_layout->getSize(slang::ParameterCategory::Uniform)
            ...
            if (reflected_count <= 0 || static_cast<size_t>(reflected_count) == SLANG_UNBOUNDED_SIZE ||
                ...
                throw std::runtime_error("Unbounded or unresolved descriptor arrays are not enabled yet.");
            }
...
void logReflection(const ShaderReflection& reflection)
{
    ...
    for (const auto& range : reflection.push_constants) {
        getLogChannel().debug("Reflected push constants (offset {}, size {})", range.offset, range.size);
    }
}
                                                       ...
                                                       std::string fragment_entry_point)
{
    ...
    diagnostics.setNull();
    checkSlangResult(
        composite->link(linked_program.writeRef(), diagnostics.writeRef()), "IComponentType::link", diagnostics);

    Slang::ComPtr<slang::IBlob> vertex_code;
    diagnostics.setNull();
    checkSlangResult(linked_program->getEntryPointCode(0, 0, vertex_code.writeRef(), diagnostics.writeRef()),
                     "IComponentType::getEntryPointCode(vertex)",
                     diagnostics);

    Slang::ComPtr<slang::IBlob> fragment_code;
    diagnostics.setNull();
    checkSlangResult(linked_program->getEntryPointCode(1, 0, fragment_code.writeRef(), diagnostics.writeRef()),
                                                     ...
                                                     std::string compute_entry_point)
{
    ...
    checkSlangResult(composite->link(linked_program.writeRef(), diagnostics.writeRef()),
                     "IComponentType::link(compute)",
                     diagnostics);

    Slang::ComPtr<slang::IBlob> compute_code;
    diagnostics.setNull();
    checkSlangResult(linked_program->getEntryPointCode(0, 0, compute_code.writeRef(), diagnostics.writeRef()),
                     "IComponentType::getEntryPointCode(compute)",
                     diagnostics);
