...
Texture2D detail::TextureAccess::create(nvrhi::IDevice& device, ResourceOwnerPtr owner,
        std::span<const std::byte> texels, uint32_t width, uint32_t height, TextureFormat format,
        bool generate_mipmaps, std::string_view debug_name) {
    ...
    texture_desc.setWidth(width)
            ...
            .setDebugName(debug_name.empty() ? "ArtiChoco Texture2D" : std::string{ debug_name });
    if (generate_mipmaps && mip_levels > 1) {
        texture_desc.setIsUAV(true).setIsTypeless(true).enableAutomaticStateTracking(
        ...
        vulkan::uploadAndGenerateNvrhiTextureMipmaps(device, impl->texture, texels,
                static_cast<size_t>(width) * bytes_per_texel, detail::toNvrhiFormat(format),
                detail::toNvrhiStorageFormat(format));
    } else {
        const std::array uploads = { vulkan::NvrhiTextureUpload{ 0, 0, texels,
            static_cast<size_t>(width) * bytes_per_texel, 0 } };
